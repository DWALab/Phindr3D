...
class interactive_points():
    """Open the image associated with a data point.

    Callback will open image associated with data point.
    Note: in 3D plot pan is hold left-click swipe,
    zoom is hold right-click swipe (zoom disabled).
    """
    def __init__(self, main_plot, projection, data, labels, feature_file, color, imageID):
        """Set the parameters associated with the plot."""
    ...
    def buildImageViewer(self, label, cur_label, index, color, feature_file, imageID):
        """Construct an image viewer window."""
        self.win = QDialog()
        ...
        #callbacks
        self.plot_img(adjustbar, main_plot, self.color, label,
            cur_label, index, feature_file, file_info, ch_info, imageID, pjt_box)
        adjustbar.valueChanged.connect(
            lambda: self.plot_img(adjustbar, main_plot, self.color, label,
                cur_label, index, feature_file, file_info,
                ch_info, imageID, pjt_box))
        slice_btn.toggled.connect(
            lambda: self.plot_img(adjustbar, main_plot, self.color, label,
                cur_label, index, feature_file, file_info, ch_info, imageID, pjt_box)
                if pjt_box.findChildren(QRadioButton)[0].isChecked() else None)
        mit_btn.toggled.connect(
            lambda: self.plot_img(adjustbar, main_plot, self.color, label,
                cur_label, index, feature_file, file_info, ch_info, imageID, pjt_box)
                if pjt_box.findChildren(QRadioButton)[1].isChecked() else None)
        montage_btn.toggled.connect(
            lambda: self.plot_img(adjustbar, main_plot, self.color, label,
                cur_label, index, feature_file, file_info, ch_info, imageID, pjt_box)
                if pjt_box.findChildren(QRadioButton)[2].isChecked() else None)
        ch_colour.clicked.connect(
            lambda: self.color_change((ch_info.text()).count("Channel_"),
                self.color, "Custom Colour Picker", "Channels",
                ["Channel_" +str(i+1) for i in range((ch_info.text()).count("Channel_"))],
                adjustbar, main_plot, label, cur_label, index, feature_file,
                file_info, ch_info, imageID, pjt_box))
        if not self.error:
            self.win.exec()
    # end buildImageViewer

    def read_featurefile(
            self, slicescrollbar, color, label, cur_label, index,
            feature_file, file_info, ch_info, imageID, pjt):
        """Read the feature file."""
        if feature_file:
            # extract image details from feature file
            data = pd.read_csv(feature_file[0], sep="\t", na_values='NaN')
            if os.path.exists(data["MetadataFile"].str.replace(
                    r'\\', '/', regex=True).iloc[0]) == False:
                self.error = True
                raise FileNotFoundError
            data = pd.read_csv(data["MetadataFile"].str.replace(
                r'\\', '/', regex=True).iloc[0], sep="\t", na_values='NaN')

            #getting metadatafile
            cols=data.columns.to_list()
            if any("Channel" in ch for ch in cols) == False \
                    or ('Stack' not in cols) or ('ImageID' not in cols):
                self.error = True
            ...
            for radio in pjt.findChildren(QRadioButton):
                if radio.isChecked():
                    ...
                    break
            file_text = "Filename: " + data['Channel_1'].str.replace(
                r'\\', '/', regex=True).iloc[meta_loc + slicescrollbar.value()]
            if len(file_text)>20:
            ...
            if 'treatmentColNameForNormalization' in cols:
                bounds = treatment_bounds(data, bounds, meta_loc,
                    data['treatmentColNameForNormalization'].iloc[0])
                if not bounds:
                    self.win.close()
            ...
            empty_img = PIL.Image.open(data['Channel_1'].str.replace(
                r'\\', '/', regex=True).iloc[meta_loc + slicescrollbar.value()]).size
            if PIL.Image.open(data['Channel_1'].str.replace(
                    r'\\', '/', regex=True).iloc[meta_loc + slicescrollbar.value()]).mode \
                    in ["CMYK","HSV", "LAB", "RGB", "RGBA", "RGBX", "YCbCr"]:
                self.error = True
                raise ColorIMG
            empty_img = np.zeros((ch_len, empty_img[1], empty_img[0], 3))
            return(data, empty_img, meta_loc, stacks, ch_len, bounds, threshold)
    # end read_featurefile

    def plot_img(
            self, slicescrollbar, img_plot, color, label, cur_label, index,
            feature_file, file_info, ch_info, imageID, pjt):
        """Reset subplots."""
        try:
            ...
            data, empty_img, metaloc, stacks, ch_len, bounds, threshold \
                = self.read_featurefile(slicescrollbar, color, label,
                cur_label, index, feature_file, file_info, ch_info, imageID, pjt)
            ...
            for x in range(1, stacks+1):
                img = merge_channels(data, empty_img, ch_len,
                    slicescrollbar.value(), color, metaloc+x-1,
                    pjt.findChildren(QRadioButton)[2].isChecked(), bounds, threshold)
                ...
                if pjt.findChildren(QRadioButton)[1].isChecked()==False or x==stacks:
                    if pjt.findChildren(QRadioButton)[1].isChecked() == False:
                        axes = img_plot.fig.add_subplot(int(np.ceil(np.sqrt(stacks))),
                            int(np.round_(np.sqrt(stacks), decimals=0)),x)
                    else:
            ...
            errorWindow("Metadata Error",
                "Cannot Find Metadatafile path. Check that Metadatafile path in Feature File is valid")
        except MissingChannelStackError:
            errorWindow("Metadata Error",
                "Metadata Extraction Failed: Channel/Stack/ImageID column(s) missing and/or invalid")
    ...
    def color_change(
            self, ch, color, win_title, col_title, labels, slicescrollbar,
            img_plot, label, cur_label, index, feature_file, file_info,
            ch_info, imageID, pjt):
        """Change colors based on user selection in color channel window."""
        colors = colorchannelWindow(ch, color, win_title, col_title, labels)
        self.color = colors.color
        self.plot_img(slicescrollbar, img_plot, self.color, label, cur_label,
            index, feature_file, file_info, ch_info, imageID, pjt)
    # end color_change

    def __call__ (self, event):
        """Right-click activation for picker."""
        if event.mouseevent.inaxes is not None \
                and event.mouseevent.button is MouseButton.RIGHT:
            # https://github.com/matplotlib/matplotlib/issues/ 19735
            # ---- code below from github open issue. wrong event.ind
            # coordinate not fixed in current version matplotlib...
            xx = event.mouseevent.x
            yy = event.mouseevent.y
            label = event.artist.get_label()
            label_ind=np.where(np.array(self.labels)==label)[0]
            # magic from https://stackoverflow.com/questions/10374930/
            #     matplotlib-annotating-a-3d-scatter-plot
            x2, y2, z2 = proj3d.proj_transform(self.data[0][label_ind[0]],
                self.data[1][label_ind[0]], self.data[2][label_ind[0]],
                self.main_plot.axes.get_proj())
            ...
            for i in range(np.shape(label_ind)[0]):
                # magic from https://stackoverflow.com/questions/10374930/
                #     matplotlib-annotating-a-3d-scatter-plot
                x2, y2, z2 = proj3d.proj_transform(self.data[0][label_ind[i]],
                    self.data[1][label_ind[i]], self.data[2][label_ind[i]],
                    self.main_plot.axes.get_proj())
                x3, y3 = self.main_plot.axes.transData.transform((x2, y2))
                # magic from https://stackoverflow.com/questions/10374930/
                #     matplotlib-annotating-a-3d-scatter-plot
            ...
            self.buildImageViewer(self.labels,label, imin,
                self.color, self.feature_file, self.imageID)
    # end __call__
# end interactive_points

class ColorIMG(Exception):
    """Derive a specific exception from the base class Exception."""
    pass